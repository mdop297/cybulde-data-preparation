...
class DatasetReader(ABC):
    ...
    def read_data(self) -> dd.core.DataFrame:
        ...
        df = self.assign_split_name_to_data_frames_and_merge(train_df, dev_df, test_df)
    ...
    def assign_split_name_to_data_frames_and_merge(
    ...
    def split_dataset(
        ...
        if stratify_column is None:
            return train_test_split(df, test_size=test_size, random_state=2907, shuffle=True) #type: ignore
        ...
        for unique_set_value in unique_column_values:
            ...
            sub_first_df, sub_second_df = train_test_split(sub_df, test_size=test_size, random_state=2907, shuffle=True)
...
class JigsawToxicCommentsDatasetReader(DatasetReader):
    ...
    def get_text_and_label_columns(self, df: dd.core.DataFrame) -> dd.core.DataFrame:
        df["label"] = df[self.columns_for_label].sum(axis=1) > 0
...
class TwitterDatasetReader(DatasetReader):
    ...
    def _read_data(self) -> tuple[dd.core.DataFrame, dd.core.DataFrame, dd.core.DataFrame]:
        ...
        train_df = train_df.rename(columns={"tweet_text": "text", "cyberbullying_type": "label"})
        train_df["label"] = (train_df["label"] != "not_cyberbullying").astype(int)

        train_df, test_df = self.split_dataset(train_df, test_size=self.test_split_ratio, stratify_column="label")
